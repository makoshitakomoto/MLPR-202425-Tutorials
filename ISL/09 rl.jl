...
begin
    ...
    function Plots.plot(env::CliffWalkingEnv)
        ...
        for i in 0:env.params.nx
            plot!([0.5, env.params.ny + 0.5], [i + 0.5, i + 0.5], c=:black)
        end
        for i in 0:env.params.ny
            plot!([i + 0.5, i + 0.5], [0.5, env.params.nx + 0.5], c=:black)
        end
...
begin
    ...
    T = reshape([onehot(3), [0, 0, 0, 0, 0.5, 0.5, 0, 0],
            [0, 0, 0.7, 0.3, 0, 0, 0, 0], [0, 0, 0, 0, 0.3, 0.7, 0, 0],
            onehot(7), onehot(7),
            onehot(8), onehot(7),
            onehot(7), onehot(7),
            onehot(7), onehot(8),
...
let
    ...
    if room < 5
        plot!([0.1, 0.1, 0.4, 0.4], [0, 0.7, 0.7, 0], w=5, c=:black)
        plot!([0.14, 0.12, 0.17], [0.37, 0.35, 0.35], w=4, c=:black)
        plot!(0.5 .+ [0.1, 0.1, 0.4, 0.4], [0, 0.7, 0.7, 0], w=5, c=:black)
        plot!(0.5 .+ [0.14, 0.12, 0.17], [0.37, 0.35, 0.35], w=4, c=:black)
    end
    if guard
        xshift = guard_door * 0.5
        plot!(xshift .+ [0.14, 0.17, 0.25, 0.33, 0.37], [0, 0, 0.3, 0, 0], w=5, c=:black)
        plot!(xshift .+ [0.25, 0.25], [0.3, 0.52], w=5, c=:black)
        scatter!(xshift .+ [0.25], [0.58], markersize=25, markerstrokewidth=0, c=:black)
        plot!(xshift .+ [0.33, 0.33, 0.18, 0.18], [0.23, 0.47, 0.47, 0.23],
            w=5, c=:black)
    end
    if gold
        x = [0.25, 0.15, 0.2, 0.3, 0.35, 0.25] .+ 0.25
        y = [0.3, 0.1, 0.2, 0.2, 0.1, 0.1]
    ...
    if room == 6
        ...
        annotate!([(0.5, 0.5, "K.O.", :red)])
    end
    _i = length(chasse.episode_recorder)
    if _i > 0
        suba = _i == 1 ? "₁" : "₂"
        subr = _i == 1 ? "₂" : "₃"
        annotate!([(0.1, 0.9, "A$suba = $(chasse.episode_recorder[end][2] == 1 ? "left" : "right")", :white),
            (0.6, 0.9, "R$subr = $(chasse.reward)", :white)])
        if _i == 2
            rs = getindex.(chasse.episode_recorder, 3)
            annotate!([(0.6, 0.8, "G₁ = R₂ + R₃ = $(join(rs, " + ")) = $(round(sum(rs), sigdigits = 2))", :white)])
        end
    end
    plot!()
end
...
let
    ...
    annotate!([(cwenv.params.ny ÷ 2, -1, "reward = $r"),
        (cwenv.params.ny ÷ 2, -0.2, "cumulative reward = $(cwenv.cumulative_reward)")])
end
